...
if if_denoi == 1
    export_fig([save_path, filesep, 'CSF_DGEmap_every',num2str(sample_num),'_',cell2mat(method_name(method_num)),'_denoi'], '-jpg', '-r200')
else
    export_fig([save_path, filesep, 'CSF_DGEmap_every',num2str(sample_num),'_',cell2mat(method_name(method_num)),'_raw'], '-jpg', '-r200')
end
...
figure, imshow(anatom_img_save/max(anatom_img_save(:))*1.5,[],'InitialMagnification','fit');
hold on
contour(mask_small_brain, 1, 'r-', 'LineWidth',3);
if if_denoi == 1
    export_fig([save_path, filesep, 'CSF_anatomy','_',cell2mat(method_name(method_num)),'_denoi'], '-jpg', '-r100')
else
    export_fig([save_path, filesep, 'CSF_anatomy','_',cell2mat(method_name(method_num)),'_raw'], '-jpg', '-r100')
end