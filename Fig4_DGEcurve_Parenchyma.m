...
hold off
% Save DGE results
saveas(gcf,[data_path,filesep,num2str(exp_num),filesep,'Parenchyma_DGE_',mask_name,'_',cell2mat(method_name(method_num)),'.jpg']);
if if_denoi == 1
    xlswrite( [data_path,filesep,int2str(exp_num),filesep,'Parenchyma_DGE_',mask_name,'_',cell2mat(method_name(method_num)),'_denoi.xls'],[time_min-base_time,dge_sig,time_min-base_time,s_fit]);
else
    xlswrite( [data_path,filesep,int2str(exp_num),filesep,'Parenchyma_DGE_',mask_name,'_',cell2mat(method_name(method_num)),'_raw.xls'],[time_min-base_time,dge_sig,time_min-base_time,s_fit]);
end
% Save fit parameters
smax = p_fit(:,1);
uin = p_fit(:,2);
if if_denoi == 1
    save_path = [data_path,filesep,num2str(exp_num),filesep,'Parenchyma_Smax_uin_R2_SNR_',mask_name,'_',cell2mat(method_name(method_num)),'_denoi.txt'];
else
    save_path = [data_path,filesep,num2str(exp_num),filesep,'Parenchyma_Smax_uin_R2_SNR_',mask_name,'_',cell2mat(method_name(method_num)),'_raw.txt'];
end
